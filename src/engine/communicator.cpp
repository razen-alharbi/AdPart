...
Communicator::Communicator(MPI::Intracomm mpi_com){
	this->comm_volume = 0;
...
void Communicator::mpi_send_string(string &msg, int dest) {
...
void Communicator::mpi_receive_string(int src, string& msg) {
...
long long Communicator::mpi_receive_long(int src) {
	long long msg;
	this->mpi_comm.Recv(&msg, 1, MPI::LONG_LONG, src, this->comm_tag);
	return msg;
}

double Communicator::mpi_receive_double(int src) {
	double msg;
	this->mpi_comm.Recv(&msg, 1, MPI::DOUBLE, src, this->comm_tag);
	return msg;
}

void Communicator::mpi_send_int_vector(vector<int> &data, int dest) {
	this->comm_volume += data.size()*sizeof(MPI::INT);
	this->mpi_comm.Send(&data[0], data.size(), MPI::INT, dest, this->comm_tag);
}

void Communicator::mpi_receive_int_vector(vector<int> &data, int from){
	this->mpi_comm.Recv(&data[0], data.size(), MPI::INT, from, this->comm_tag);
}
void Communicator::mpi_send_long(long long msg, int dest) {
	if (dest == SEND_TO_ALL) {
...
void Communicator::mpi_send_double(double msg, int dest) {
	if (dest == SEND_TO_ALL) {
...
void Communicator::send_command(net_cmd cmd, vector<string> &params, int dest) {
...
int Communicator::receive_command(net_cmd& cmd, vector<string>& cmd_params) {
...
string Communicator::translate_command_to_string(net_cmd cmd) {
	string command;
	switch (cmd) {
	case CMD_LOAD_DATA:
	...
	case CMD_COMPUTE_PREDICATE_STATS:
		command = "CMD_COMPUTE_PREDICATE_STATS";
		break;
	case CMD_CLEAN_VERTS_DATA:
		command = "CMD_CLEAN_VERTS_DATA";
		break;
	case CMD_EXCHANGE_VERTS_STATS:
		command = "CMD_EXCHANGE_VERTS_STATS";
		break;
	case CMD_GET_SUBQUERY_CARDINALITY:
		command = "CMD_GET_QUERY_MASSES";
		break;
	case CMD_JOIN_WITHOUT_COMMUNICATION:
		command = "CMD_JOIN_WITHOUT_COMMUNICATION";
		break;
	case CMD_JOIN_WITH_COMMUNICATION:
		command = "CMD_JOIN_WITH_COMMUNICATION";
		break;
	case CMD_SOLVE_STAR_QUERY:
		command = "CMD_SOLVE_STAR_QUERY";
		break;
	case CMD_START_PHD:
		command = "CMD_START_PHD";
		break;
	case CMD_EXECUTE_QUERY_IN_PARALLEL:
		command = "CMD_EXECUTE_QUERY_IN_PARALLEL";
		break;
	case CMD_STORE_STATS:
		command = "CMD_STORE_STATS";
		break;
	case CMD_EVICT:
		command = "CMD_EVICT";
		break;
	case CMD_COMM_VOL:
		command = "CMD_COMM_VOL";
		break;
		/*case CMD_KILL:
		command = "CMD_KILL";
		break;
	...
	case CMD_GET_TRIPLE_LOAD:
		command = "CMD_GET_TRIPLE_LOAD";
		break;
	case CMD_GET_QUERY_MASSES:
		command = "CMD_GET_QUERY_MASSES";
		break;
	case CMD_PREPARE_JOIN_COL:
		command = "CMD_PREPARE_JOIN_COL";
		break;
	case CMD_DISTRIBUTE_JOIN_COLUMN:
		command = "CMD_DISTRIBUTE_JOIN_COLUMN";
		break;
	case CMD_BUILD_CANDIDATE_LISTS:
		command = "CMD_BUILD_CANDIDATE_LISTS";
		break;
	case CMD_DISTRIBUTE_CANDIDATE_LISTS:
		command = "CMD_DISTRIBUTE_CANDIDATE_LISTS";
		break;
	case CMD_PARALLEL_EXEC_JOIN:
		command = "CMD_PARALLEL_EXEC_JOIN";
		break;
	case CMD_CLEANUP:
		command = "CMD_CLEANUP";
		break;
	case CMD_GET_QUERY_RESULT:
		command = "CMD_GET_QUERY_RESULT";
		break;
	case CMD_REMOVE_REPLICATED_DATA:
		command = "CMD_REMOVE_REPLICATED_DATA";
		break;
	case CMD_EXECUTE_QUERY_IN_PARALLEL:
		command = "CMD_EXECUTE_QUERY_IN_PARALLEL";
		break;
	...
	case CMD_CLEAN_VERTS_DATA:
		command = "CMD_CLEAN_VERTS_DATA";
		break;
	case CMD_GET_DATA_COMM:
		command = "CMD_GET_DATA_COMM";
		break;
	case CMD_EXPLORE:
		command = "CMD_EXPLORE";
		break;
	case CMD_JOIN:
		command = "CMD_JOIN";
		break;
	case CMD_SOLVE_FINALIZE_STAR:
		command = "CMD_SOLVE_FINALIZE_STAR";
		break;
	case CMD_FINALIZE:
		command = "CMD_FINALIZE";
		break;*/
	default:
...
int Communicator::get_worker_id(){
	return this->rank;
}

int Communicator::get_num_workers(){
	return num_workers;
}

void Communicator::wait_for_clients() {
...
void Communicator::mpi_all_to_all(vector<vector<int> >&src, vector<vector<int> > &recvd) {
